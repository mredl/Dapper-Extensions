...
namespace DapperExtensions
{
    ...
    public interface IAsyncDatabase : IBaseDatabase
    {
        ...
        Task<IEnumerable<T>> GetListAsync<T>(object predicate, IList<ISort> sort, IDbTransaction transaction, int? commandTimeout = null, bool buffered = true) where T : class;
        Task<IEnumerable<T>> GetListAsync<T>(object predicate = null, IList<ISort> sort = null, int? commandTimeout = null, bool buffered = true) where T : class;
        Task<IEnumerable<T>> GetPageAsync<T>(object predicate, IList<ISort> sort, int page, int resultsPerPage, IDbTransaction transaction, int? commandTimeout = null, bool buffered = true) where T : class;
        Task<IEnumerable<T>> GetPageAsync<T>(object predicate, IList<ISort> sort, int page, int resultsPerPage, int? commandTimeout = null, bool buffered = true) where T : class;
        Task<IEnumerable<T>> GetSetAsync<T>(object predicate, IList<ISort> sort, int firstResult, int maxResults, IDbTransaction transaction, int? commandTimeout, bool buffered) where T : class;
        Task<IEnumerable<T>> GetSetAsync<T>(object predicate, IList<ISort> sort, int firstResult, int maxResults, int? commandTimeout, bool buffered) where T : class;
        ...
        Task<IClassMapper> GetMapAsync<T>() where T : class;
        void ClearCache();
    ...
    public class AsyncDatabase : BaseDatabase, IAsyncDatabase
    {
        ...
        public async virtual Task<T> GetAsync<T>(dynamic id, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            return await _dapper.GetAsync<T>(Connection, id, transaction, commandTimeout);
        }

        public async virtual Task<T> GetAsync<T>(dynamic id, int? commandTimeout) where T : class
        {
        ...
        public async virtual Task<dynamic> InsertAsync<T>(T entity, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            return await _dapper.InsertAsync(Connection, entity, transaction, commandTimeout);
        }

        public async virtual Task<dynamic> InsertAsync<T>(T entity, int? commandTimeout) where T : class
        {
            return await _dapper.InsertAsync(Connection, entity, _transaction, commandTimeout);
        }

        public async virtual Task<bool> UpdateAsync<T>(T entity, IDbTransaction transaction, int? commandTimeout, bool ignoreAllKeyProperties) where T : class
        {
            return await _dapper.UpdateAsync(Connection, entity, transaction, commandTimeout, ignoreAllKeyProperties);
        }

        public async virtual Task<bool> UpdateAsync<T>(T entity, int? commandTimeout, bool ignoreAllKeyProperties) where T : class
        {
            return await _dapper.UpdateAsync(Connection, entity, _transaction, commandTimeout, ignoreAllKeyProperties);
        }

        public async virtual Task<bool> DeleteAsync<T>(T entity, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            return await _dapper.DeleteAsync(Connection, entity, transaction, commandTimeout);
        }

        public async virtual Task<bool> DeleteAsync<T>(T entity, int? commandTimeout) where T : class
        {
            return await _dapper.DeleteAsync(Connection, entity, _transaction, commandTimeout);
        }

        public async virtual Task<bool> DeleteAsync<T>(object predicate, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            return await _dapper.DeleteAsync<T>(Connection, predicate, transaction, commandTimeout);
        }

        public async virtual Task<bool> DeleteAsync<T>(object predicate, int? commandTimeout) where T : class
        {
            return await _dapper.DeleteAsync<T>(Connection, predicate, _transaction, commandTimeout);
        }

        public async virtual Task<IEnumerable<T>> GetListAsync<T>(object predicate, IList<ISort> sort, IDbTransaction transaction, int? commandTimeout, bool buffered) where T : class
        {
            return await _dapper.GetListAsync<T>(Connection, predicate, sort, transaction, commandTimeout, buffered);
        }

        public async virtual Task<IEnumerable<T>> GetListAsync<T>(object predicate, IList<ISort> sort, int? commandTimeout, bool buffered) where T : class
        {
            return await _dapper.GetListAsync<T>(Connection, predicate, sort, _transaction, commandTimeout, buffered);
        }

        public async virtual Task<IEnumerable<T>> GetPageAsync<T>(object predicate, IList<ISort> sort, int page, int resultsPerPage, IDbTransaction transaction, int? commandTimeout, bool buffered) where T : class
        {
            return await _dapper.GetPageAsync<T>(Connection, predicate, sort, page, resultsPerPage, transaction, commandTimeout, buffered);
        }

        public async virtual Task<IEnumerable<T>> GetPageAsync<T>(object predicate, IList<ISort> sort, int page, int resultsPerPage, int? commandTimeout, bool buffered) where T : class
        {
            return await _dapper.GetPageAsync<T>(Connection, predicate, sort, page, resultsPerPage, _transaction, commandTimeout, buffered);
        }

        public async virtual Task<IEnumerable<T>> GetSetAsync<T>(object predicate, IList<ISort> sort, int firstResult, int maxResults, IDbTransaction transaction, int? commandTimeout, bool buffered) where T : class
        {
            return await _dapper.GetSetAsync<T>(Connection, predicate, sort, firstResult, maxResults, transaction, commandTimeout, buffered);
        }

        public async virtual Task<IEnumerable<T>> GetSetAsync<T>(object predicate, IList<ISort> sort, int firstResult, int maxResults, int? commandTimeout, bool buffered) where T : class
        {
            return await _dapper.GetSetAsync<T>(Connection, predicate, sort, firstResult, maxResults, _transaction, commandTimeout, buffered);
        }

        public async virtual Task<int> CountAsync<T>(object predicate, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            return await _dapper.CountAsync<T>(Connection, predicate, transaction, commandTimeout);
        }

        public async virtual Task<int> CountAsync<T>(object predicate, int? commandTimeout) where T : class
        {
            return await _dapper.CountAsync<T>(Connection, predicate, _transaction, commandTimeout);
        }

        public async virtual Task<IMultipleResultReader> GetMultipleAsync(GetMultiplePredicate predicate, IDbTransaction transaction, int? commandTimeout)
        {
            return await _dapper.GetMultipleAsync(Connection, predicate, transaction, commandTimeout);
        }

        public async virtual Task<IMultipleResultReader> GetMultipleAsync(GetMultiplePredicate predicate, int? commandTimeout)
        {
            return await _dapper.GetMultipleAsync(Connection, predicate, _transaction, commandTimeout);
        }

        public virtual Task<Guid> GetNextGuidAsync()
        {
            return Task.FromResult(_dapper.SqlGenerator.Configuration.GetNextGuid());
        }

        public virtual Task<IClassMapper> GetMapAsync<T>() where T : class
        {
            return Task.FromResult(_dapper.SqlGenerator.Configuration.GetMap<T>());
        }
